...
impl Add for ClassicCard {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        res.iter_mut()
            .enumerate()
            .for_each(|(i, x)| *x = (self.0[i] + rhs.0[i]) % 3);
        Self::Output::new(res)
    }
}
    ...
    fn draw(&self, frame: &mut canvas::Frame<Renderer>) {
        {
            ...
            let ypositions: &[f32] = match self.0[1] {
                0 => &[0.5],
                1 => &[0.35, 0.65],
                2 => &[0.2, 0.5, 0.8],
                _ => panic!(),
            ...
            let shape = standard_shape(self.0[2]);
