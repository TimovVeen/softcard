...
impl<Deck: Iterator<Item = SymCard<4>> + Default> SymSet<Deck> {
    pub fn new() -> Self {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        container(responsive(|size| {
            let expected_width =
                (size.height - GRID_SPACING) * CARD_ASPECT * 2.5 + 4. * GRID_SPACING;
            ...
            let stats = container(
                if self.finished {
                    ...
                    widget::column![
                        widget::text!("Remaining cards: {}", 23 - self.card_head),
    ...
    fn resolve_selection(&mut self) {
        ...
        info!("You got a set!");
        if self.selection.len() + self.card_head >= 23 {
            self.finished = true;
...
impl<Deck: Iterator<Item = SymCard<4>> + Default> Default for SymSet<Deck> {
    fn default() -> Self {
        Self::new()
    }
}
